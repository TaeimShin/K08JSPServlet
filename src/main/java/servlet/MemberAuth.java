...
public class MemberAuth extends HttpServlet {
	...
	//서블릿객체 생성시 최초 1번만 호출되는 수명주기 메서드
	@Override
	public void init() throws ServletException {
		/*
		 application 내장객체를 얻어옴. 모델2방식에서는 서블릿이 먼저 요청을
		 받기 떄문에 모델 1 방식과 같이 JSP에서 매개변수 형태로 내장객체를 전달할 수 없다.
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(memberName != null) {
			...
			if(admin_id.equals(id)) //초기화 파라미터와 비교해서 일치하면..
				...
				req.setAttribute("autoMessage", "귀하는 회원이 아닙니다.");
		}
		//View로 포워드
